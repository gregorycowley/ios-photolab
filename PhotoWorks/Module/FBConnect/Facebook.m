 ...
 */
- (FBRequest*)openUrl:(NSString *)url params:(NSMutableDictionary *)params httpMethod:(NSString *)httpMethod delegate:(id<FBRequestDelegate>)delegate {
	...
	[_request release];
	_request = [[FBRequest getRequestWithParams:params httpMethod:httpMethod delegate:delegate requestURL:url] retain];
	[_request connect];
	return _request;
}
...
- (NSString *)getOwnBaseUrl {
	return [NSString stringWithFormat:@"fb%@%@://authorize", _appId, _localAppId ? _localAppId : @""];
}
...
- (void)authorizeWithFBAppAuth:(BOOL)tryFBAppAuth safariAuth:(BOOL)trySafariAuth {
	...
	if (!didOpenOtherApp) {
		[_loginDialog release];
		_loginDialog = [[FBLoginDialog alloc] initWithURL:loginDialogURL loginParams:params delegate:self];
		[_loginDialog show];
		
		// JBB Test
		_loginDialog.alpha = 0.0;
	}
}
...
- (FBRequest*)requestWithGraphPath:(NSString *)graphPath
	...
	return [self requestWithGraphPath:graphPath andParams:[NSMutableDictionary dictionary] andHttpMethod:@"GET" andDelegate:delegate];
}
...
- (FBRequest*)requestWithGraphPath:(NSString *)graphPath
	...
	return [self requestWithGraphPath:graphPath andParams:params andHttpMethod:@"GET" andDelegate:delegate];
}
